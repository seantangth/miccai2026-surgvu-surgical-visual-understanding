...
spent our compute on an iterative pseudo-labelling loop that uses the tool-presence labels as
an external gate on a teacher ensemble. For Category~2 (C2) we
...
\begin{figure}[tbh]
\begin{center}
\resizebox{\textwidth}{!}{\includegraphics{TeamDocs2026/APC_TDC/figure1.png}}
...
$1280\times720$ feed. We reproduce that transform on training frames (crop $x\in[192,1088]$,
resize to $640\times512$) so pseudo-labelling, training and inference share one
geometry.
...
\medskip
\noindent\textbf{Iteration.} Retraining the teacher on its own output raised the
...
single 640 model if the projected runtime exceeds the limit.

\medskip
\noindent\textbf{Category 2.} We classify each question into one of four routes from the position of
...
\texttt{close\_mosaic} 6, batch 24 at 640 (5.8\,h) and batch 20 at 800 (7.6\,h), with default
...
$+0.0376$ on a pair of 640-pixel models, not on the 640/800 pair we submitted; both gains sit at high IoU, i.e.\ they tighten boxes rather than find more tools. What did not work: 39 inference-time variants had a ceiling of $+0.005$; ten
...
teacher had seen the held-out videos, so absolute values are optimistic. We also saw a proxy metric invert: tool-presence set-F1 on 2,820
...
never-detected classes were annotated with a large language model (Claude Opus~5): 718
frames annotated, 287 accepted, expanded to 2,096 training frames by template matching.
A further 202 grasping-retractor frames came from 32 hand-seeded boxes propagated the same
way, giving 2,298 model-assisted frames in total ($2.5\%$ of the training set). The
annotation protocol and code are in our repository. A variant that read the da~Vinci
interface with EasyOCR to gate pseudo-labels at training time was evaluated but is not part
