...
FR = ROOT / "1_data/processed/rare_frames_640"; RES = ROOT / "1_data/processed/opus_bbox/results"
OUT = ROOT / "1_data/processed/pseudo_opus_v1"; MONT = ROOT / "1_data/processed/opus_bbox/montage"
...
weak = json.load(open(ROOT / "1_data/processed/rare_frames_weak_labels_v3.json"))
W, H = 640, 512
def iou(a, b):
...
for f in sorted(glob.glob(str(RES / "*.json"))):
    ...
    key = key.replace("C", "")          # tipupC / prograspC → tipup / prograsp（選擇題版）
    for r in rows:
...
def dets(vid, sec):
    k = (vid, sec)
    if k not in cache:
        r = model.predict(str(FR / vid / f"sec_{sec:06d}.jpg"), conf=0.05, imgsz=640, max_det=300, device="mps", verbose=False)[0]
